...
import { putApiFunction, getAddressByCepla, getApiClients, getApiFunction, deleteApiFunction } from '../../../providers/api'
...
function BlockDialog(props) {
    ...
    const send = async () => {
        setLoading(true);
        await deleteApiFunction(props.id, { justification: justfy });
        props.handle(1);
        props.handleClose();
        setjustfy('');
        setLoading(false);
...
class EditFunction extends Component {
    ...
    async componentDidMount() {
        ...
        let data = await getApiFunction({}, this.props.match.params.id);
    ...
    render() {
        ...
        const request = async (state, data) => {
            ...
            let response = await putApiFunction(this.props.match.params.id, data);
        ...
        const forms = this.state.data.id == undefined ? [] : [
            {
                title: 'Perfil',
                fields: [
                    { column: 'name', label: 'Nome', type: 'text', validate: { max: 20, min: 1, required: true }, value: 1, flexBasis: "45%", value: this.state.data.name },
                 ]
            },
        ]
                ...
                <AppBar position="static" style={{ padding: 10, marginTop: 10, marginBottom: 10 }}>
                    <Toolbar>
                        <Typography variant="h6"  style={{flexGrow: 1}}>
                            <HomeIcon />  <span>Editar / Perfil</span>
                        </Typography>
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(EditFunction))
